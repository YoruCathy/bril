    ...
    if ("op" in line) {
      ...
      switch (action.action) {
        case "end": {
          ...
          if (tracing && isTracingFunc && traceInstrs.length > 0) {
            // finalize global trace for this run of main
            globalTrace = {
              func: func.name,
              trace: traceInstrs,
            };
...
function recordTraceInstr(
  ...
  if (line.op === "br") {
    ...
    if (typeof condVal !== "boolean") {
      throw error(`branch condition ${condVar} is not bool at runtime`);
    }

    if (condVal) {
      // Took the "then" branch: guard that cond stays true
      traceInstrs.push({
        op: "guard",
        args: [condVar],
        labels: [bailLabel],
      ...
      // Took the "else" branch: guard that cond stays false
      const tmp = freshTraceTmp();
      traceInstrs.push(
        {
          op: "not",
          args: [condVar],
  ...
  // Otherwise, record the instruction as-is (shallow copy)
  traceInstrs.push({ ...line });
}
...
function evalProg(prog: bril.Program) {
  ...
  if (main === null) {
    ...
    // Silly argument parsing to find the `-p` and `-t` flags.
